...
if __name__ == "__main__":
    ...
    predictions_seg = os.path.join(input_path, "predictions_seg.json")
    PREDICTIONS_JSON = predictions_both if os.path.exists(predictions_both) else predictions_seg
    # Handle missing predictions file
    if not os.path.exists(PREDICTIONS_JSON):
        print(f"ERROR: No predictions file found. Expected: {predictions_both} or {predictions_seg}")
        results = {
            "F1[50]": 0.0,
            "F2[50]": 0.0,
            "F1[40:95]": 0.0,
    ...
    all_results = evaluate_seg(gt_coco, pred_coco, all_thresholds)
